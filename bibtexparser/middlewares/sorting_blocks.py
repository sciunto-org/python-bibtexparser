...
from bibtexparser.model import Entry, Block, String, Preamble, ImplicitComment, ExplicitComment

DEFAULT_BLOCK_TYPE_ORDER = (
    String, Preamble, Entry, ImplicitComment, ExplicitComment
)
...
class _BlockJunk:
    ...
    def main_block_type(self) -> type:
        try:
            ...
            raise RuntimeError("Block junk must contain at least one block. "
    ...
    def _verify_all_types_are_block_types(sort_order):
        for t in sort_order:
            if not issubclass(t, Block):
                raise ValueError("Sort order must only contain Block subclasses, "
                                 f"but got {str(t)}")
    ...
    def transform(self, library: Library) -> Library:
        ...
        if self._preserve_comments_on_top:
            ...
            def _sort_key(block_junk):
                ...
                try:
                    return self._block_type_order.index(block_junk.main_block_type), block_junk.sort_key
                except ValueError:
            ...
            return Library(blocks = [block for block_junk in block_junks for block in block_junk.blocks])
        else:
