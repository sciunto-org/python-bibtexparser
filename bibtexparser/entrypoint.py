import warnings
from typing import Iterable, Optional, Union, TextIO, List
...
def _build_stack(parse_stack: Optional[Iterable[Middleware]],
                 append_middleware: Optional[Iterable[Middleware]]) -> List[Middleware]:
    if parse_stack is not None and append_middleware is not None:
        raise ValueError("Provided both parse_stack and append_middleware."
    ...
    if len(stack_types_intersect) > 0:
        warnings.warn("Some middleware passed in append_middleware are "
...
def parse_file(path: str,
    ...
    with open(path) as f:
        bibtex_str = f.read()
        return parse_string(bibtex_str,
                            parse_stack=parse_stack,
                            append_middleware=append_middleware)


def write_file(file: Union[str, TextIO],
               library: Library,
               parse_stack: Optional[Iterable[Middleware]] = None,
               append_middleware: Optional[Iterable[Middleware]] = None,
               bibtex_format: Optional[BibtexFormat] = None) -> None:
    ...
    bibtex_str = write_string(library=library,
                              parse_stack=parse_stack,
                              append_middleware=append_middleware,
